...
# This script deletes an account, while saving the account's home directory.
ARCHIVE_DIR='/archive'

usage() {
  # Display the usage & exit.
...
do 
  ...
  then
    ...
    then
      ...
      if [[ "${?}" -ne 0 ]]
      then
        echo "The archive dir ${ARCHIVE_DIR} could not be created."
        exit 1
      fi
    fi


    # Archive the user's home directory & move it into the ARCHIVE_DIR
